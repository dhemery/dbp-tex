...
\def\textblock{
  \message{^^Jshipping textblock at \the\vsize (content \the\ht255+\the\dp255)^^J}
  \box255
}
...
\def\rememberpagebreak{
  ...
  \global\bestdiagnosis=\diagnosis
  \global\bestadjustment=\vsize
  \global\advance\bestadjustment by -\textblockheight
  \message{, saving pass \the\deadcycles, \the\bestdiagnosis, \the\bestadjustment}
}

\def\rememberbestpagebreak{
  \ifnum\diagnosis<\bestdiagnosis
    \rememberpagebreak
  \fi
}
...
\def\tryagainwithtextblock#1{
  ...
  \message{, will try \the\vsize}
  \unvbox255
  \penalty\outputpenalty
}

\def\balancedspreads{
  \message{^^Jpage \the\pageno, pass \the\deadcycles, \the\vsize, \the\outputpenalty, content \the\ht255+\the\dp255}
  \diagnosis=\outputpenalty
  ...
  \else
    ...
    \or % 4: settling for least bad
      \message{, shipping anyway, using pass \the\bestpass}
